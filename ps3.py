...
SCRABBLE_LETTER_VALUES = {
    '*': 0, 'a': 1, 'b': 3, 'c': 3, 'd': 2, 'e': 1, 'f': 4, 'g': 2, 'h': 4, 'i': 1, 'j': 8, 'k': 5, 'l': 1, 'm': 3, 'n': 1,
...
def get_word_score(word, n):
    ...
    if word:
        for letter in word:
            factor1 += SCRABBLE_LETTER_VALUES.get(letter)
    factor2 = max(1, 7 * len(word) - 3 * (n - len(word)))
...
def is_valid_word(word, hand, word_list):
    ...
    if '*' not in word:
        if word in word_list:
            for i in range(len(word)):
                if word[i] in hand and hand[word[i]] >= word.count(word[i]):
                    sum += 1
            return sum == len(word)
        else:
            return False
        ...
        for i in range(len(VOWELS)):
            if word.replace('*', VOWELS[i]) in word_list:
                for j in range(len(word)):
                    if word[j] in hand and hand[word[j]] >= word.count(word[j]):
                        sum += 1
                        if sum == len(word):
                            return True
                return False  # in case no vowel-replaced word is qualified for the current hand
